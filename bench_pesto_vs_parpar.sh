#!/usr/bin/env bash
# PAR2 benchmark: parmesan vs parpar
#
# Usage: ./bench_pesto_vs_parpar.sh [SIZE_GB ...]
#   Default sizes: 5 (GB)
#   Example:       ./bench_pesto_vs_parpar.sh 1 2
#
# Both tools are run with 10% recovery, slice target matching parmesan's default.
# Source files are created as sparse files (instant, no real I/O on creation).
# PAR2 output goes to OUTDIR/parmesan/ and OUTDIR/parpar/ respectively.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PARMESAN="$SCRIPT_DIR/target/release/parmesan"
OUTDIR="$SCRIPT_DIR/bench_out"
RECOVERY_PCT=10
# Parpar slice target: match parmesan's TARGET_PAR2_SLICES (pesto-par2: ops.rs = 1000)
PARPAR_SLICES=1000
...
# speedup_pct <parmesan_ms> <parpar_ms>  →  "+12.3%" or "-5.1%" relative to parpar
speedup_pct() {
    local p=$1 a=$2
...
[[ -x "$PARMESAN" ]] || fail "parmesan binary not found at $PARMESAN — run: cargo build --release"
command -v parpar >/dev/null 2>&1 || fail "parpar not found in PATH"

mkdir -p "$OUTDIR/parmesan" "$OUTDIR/parpar"

# ── header ────────────────────────────────────────────────────────────────────

echo
bold "BENCHMARK: parmesan vs parpar — PAR2 generation"; echo
echo "  Recovery : ${RECOVERY_PCT}%"
echo "  Slices   : ~${PARPAR_SLICES} (parpar -s${PARPAR_SLICES}; parmesan auto-targets same range)"
...
run_bench() {
    local tool=$1   # "parmesan" or "parpar"
    ...
    if [[ "$tool" == "parmesan" ]]; then
        # parmesan writes par2 files next to the source; we cd into OUTDIR/parmesan
        # and use a symlink so output lands there instead of beside the source.
        ln -sf "$src" "$OUTDIR/parmesan/${stem}"
        (
            cd "$OUTDIR/parmesan"
            "$PARMESAN" --recovery-pct "$RECOVERY_PCT" "${stem}"
...
for size_gb in "${SIZE_LIST[@]}"; do
    ...
    for tool in parmesan parpar; do
    ...
    if [[ "${RES_OK[$p_key]}" -eq 0 && "${RES_OK[$a_key]}" -eq 0 ]]; then
        ...
        if [[ "$sp" == +* ]]; then
            printf "  → parmesan faster by "; green "$sp"; echo " vs parpar"
        else
            printf "  → parmesan slower by "; red "$sp"; echo " vs parpar"
        fi
    fi
    echo
done
...
hr
printf "%-12s  %-12s  %-12s  %-14s\n" "Size" "parmesan" "parpar" "pms vs parpar"
hr
